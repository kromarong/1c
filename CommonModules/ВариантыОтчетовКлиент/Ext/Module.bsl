...
Процедура ОбработатьДействияСообщения(Сообщение, Действие, ДополнительныеПараметры) Экспорт 
	...
	Если Действие = ВариантыОтчетовКлиентСервер.КодДействияСообщенияПрименитьПереданныеНастройки() Тогда 
...
// Возвращает признак выполнения события ОбработкаОповещения.
//
// Параметры:
//  ИмяСобытия - Строка - имя обработчика события,
//      являющего критерием для определения возвращаемого значения.
//
// Возвращаемое значение:
//   Булево - признак того, что событие обработано.
//
Функция ОповещениеОбработано(ИмяСобытия) Экспорт 
	
	События = Новый Структура("Запись_НаборКонстант");
	События.Вставить(ИмяСобытияИзменениеВарианта());
	
	Возврат События.Свойство(ИмяСобытия);
