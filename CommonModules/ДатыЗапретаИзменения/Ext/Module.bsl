...
Функция ПользовательСРазрешенимДатыЗапретаРедактирования(Пользователь, ДПроведения) Экспорт//вмс

	Запрос_Текст=
	"ВЫБРАТЬ
	|	ПользователиСРазрешенимДатыЗапретаРедактирования.НаДатуДокумента КАК НаДатуДокумента,
	|	ПользователиСРазрешенимДатыЗапретаРедактирования.НаДатуПроведения КАК НаДатуПроведения
	|ИЗ
	|	РегистрСведений.ПользователиСРазрешенимДатыЗапретаРедактирования КАК ПользователиСРазрешенимДатыЗапретаРедактирования
	|ГДЕ
	|ПользователиСРазрешенимДатыЗапретаРедактирования.НаДатуДокумента = &Пользователь";
	
	если   ДПроведения тогда
		Запрос_Текст=СтрЗаменить(Запрос_Текст,
		"ПользователиСРазрешенимДатыЗапретаРедактирования.НаДатуДокумента = &Пользователь",
		"ПользователиСРазрешенимДатыЗапретаРедактирования.НаДатуПроведения = &Пользователь");
	Конецесли;
...
Функция НайденЗапретИзмененияДанных(Знач ДанныеДляПроверки,
	...
	если  ПользовательСРазрешенимДатыЗапретаРедактирования(Пользователь, ложь) тогда  //вмс
		ЗапретыИзменения.очистить();
	конецесли;
	
	если ЗапретыИзменения.Количество() = 0  тогда   //вмс
		если ЭкспертизаИСертификация.получитьДатаЗапретаПроведенныхДокументов(ОписаниеДанных.Данные,ложь) тогда
			если типзнч(ОписаниеДанных.Данные)=тип("ДокументОбъект.Заявка") тогда
				если  ЭкспертизаИСертификация.получитьДатаЗапретаПроведенныхДокументов(ОписаниеДанных.Данные,истина)>=ОписаниеДанных.Данные.ДатаПроведения тогда
					Для Каждого Данные Из ДанныеДляПроверки Цикл 
						Строка = ЗапретыИзменения.Добавить();
						Строка.Данные  = Данные;
						Строка.Раздел  = РазделЗапрета;
						Строка.Объект  = ОбъектЗапрета;
						Строка.Адресат = Пользователь;
						Строка.Дата    = ЭкспертизаИСертификация.получитьДатаЗапретаПроведенныхДокументов(ОписаниеДанных.Данные,истина);
					КонецЦикла;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		
		если  ПользовательСРазрешенимДатыЗапретаРедактирования(Пользователь, истина) тогда  //вмс
			ЗапретыИзменения.очистить();
		конецесли;
