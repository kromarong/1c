			...
			Для каждого СтрокаТаблицы Из ПредопределенныеДанные Цикл
				...
				Если ЛокализуемыеРеквизитыОбъекта.Количество() > 0 Тогда
					Если МультиязычныеСтрокиВРеквизитах Тогда
						...
						КодЯзыка = ОбщегоНазначения.КодЯзыкаИнформационнойБазы();
...
Процедура ЗарегистрироватьПредопределенныеЭлементыДляОбновления(Параметры) Экспорт
	...
	ОсновнойЯзык = ОбщегоНазначения.КодЯзыкаИнформационнойБазы();
	...
	Для каждого ОбъектМетаданных Из ОбъектыСПредопределеннымиЭлементами Цикл
		...
		Для каждого РеквизитОбъекта Из ЛокализуемыеРеквизитыОбъекта Цикл
			Имя = "Таблица." + РеквизитОбъекта.Ключ;
			ИмяРеквизитОбъекта.Добавить(Имя);
			ИмяРеквизитОбъекта.Добавить(Имя + "Язык1");
			ИмяРеквизитОбъекта.Добавить(Имя + "Язык2");
		КонецЦикла;
