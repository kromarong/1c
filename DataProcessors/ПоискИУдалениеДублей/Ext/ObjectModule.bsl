...
Функция ГруппыДублей(Знач ПараметрыПоиска, Знач ЭталонныйОбъект = Неопределено) Экспорт
	...
	ИспользоватьПрикладныеПравила = ПараметрыПоиска.УчитыватьПрикладныеПравила
		И ЕстьПрикладныеПравилаОбластиПоискаДублей(ПолноеИмяОбъектаМетаданных);
	...
	Для Каждого КлючЗначение Из СтруктураПолейИдентичности Цикл
		Если ТаблицаДублей.Колонки.Найти(КлючЗначение.Ключ) = Неопределено Тогда
			ТаблицаДублей.Колонки.Добавить(КлючЗначение.Ключ);
		КонецЕсли;
	КонецЦикла;
	...
	Если ТаблицаДублей.Колонки.Найти("Код") = Неопределено Тогда
		ТаблицаДублей.Колонки.Добавить("Код");
	КонецЕсли;
	Если ТаблицаДублей.Колонки.Найти("Наименование") = Неопределено Тогда
		ТаблицаДублей.Колонки.Добавить("Наименование");
	КонецЕсли;
	Если ТаблицаДублей.Колонки.Найти("Родитель") = Неопределено Тогда
		ТаблицаДублей.Колонки.Добавить("Родитель");
	КонецЕсли;
	Если ТаблицаДублей.Колонки.Найти("ПометкаУдаления") = Неопределено Тогда
		ТаблицаДублей.Колонки.Добавить("ПометкаУдаления");
	КонецЕсли;
