...
Процедура ИмитироватьДлительноеДействие(НомерОперации, ДлительностьРасчета, ВыводитьПрогрессВыполнения)
	...
	Пока ТекущаяДатаСеанса() < ВремяНачала + ДлительностьРасчета Цикл
		Пауза(1);
		ПрошлоВремени = ТекущаяДатаСеанса() - ВремяНачала;
		Процент = Цел(ПрошлоВремени / ДлительностьРасчета * 100);
		Этап = Мин(Цел(ПрошлоВремени / (ДлительностьРасчета / 3)) + 1, 3); // Имитация операции в 3 этапа.
