...
Процедура ПосмотретьКодВариантПервый(Команда)
	...
	ОткрытьФорму("Обработка.ВыводСообщенийПользователю.Форма.ПримерКода",
...
Процедура ПоказатьСообщениеСвязанноеСРеквизитомОбъектаИнформационнойБазы(Команда)
	...
	Если НЕ ПоказатьСообщениеСвязанноеСРеквизитомОбъектаИнформационнойБазСервер() Тогда
		...
		ПоказатьПредупреждение(ОписаниеОповещения, НСтр("ru = 'Для выполнения теста необходимо предварительно создать хотя бы один документ СчетНаОплатуПокупателю.'"));
	КонецЕсли;
...
Процедура ПосмотретьКодВариантТретий(Команда)
	...
	ОткрытьФорму("Обработка.ВыводСообщенийПользователю.Форма.ПримерКода",
		Новый Структура("ПримерКода", "ОбщегоНазначения.СообщитьПользователю(
		|	НСтр(""ru = 'Пример сообщения, связанного с реквизитом ""Ответственный"" документа СчетНаОплатуПокупателю 
...
Процедура ПосмотретьКодВариантЧетвертый(Команда)
	...
	ОткрытьФорму("Обработка.ВыводСообщенийПользователю.Форма.ПримерКода",
		Новый Структура("ПримерКода", "ОбщегоНазначения.СообщитьПользователю(
		|	НСтр(""ru = 'Тестовое сообщение об ошибке, связанное с реквизитом ""Ответственный"" документа СчетНаОплатуПокупателю
		|		|(ссылка на объект).'""),
		|	Ссылка,
		|	""Ответственный"")"));
...
Процедура ПоказатьСообщениеСвязанноеСРеквизитомОбъектаИнформационнойБазыЗавершение(ДополнительныеПараметры) Экспорт
	...
	ОткрытьФорму("Документ.СчетНаОплатуПокупателю.Форма.ФормаСписка");
