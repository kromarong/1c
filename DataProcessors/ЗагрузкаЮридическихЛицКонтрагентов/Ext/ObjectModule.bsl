...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	Команда.Модификатор   = Метаданные.Справочники.Контрагенты.ПолноеИмя();
...
Процедура СопоставитьКонтрагентов(ЗагружаемыеДанные)
	...
	Для Каждого Строка Из ЗагружаемыеДанные Цикл
		Строка.ОбъектСопоставления = Справочники.Контрагенты.НайтиПоРеквизиту("ИНН", СокрЛП(Строка.ИНН));
		Если Строка.ОбъектСопоставления = Неопределено Тогда
			Строка.ОбъектСопоставления = Справочники.Контрагенты.НайтиПоНаименованию(Строка.Наименование, Истина);
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗаписатьКонтрагента(СтрокаТаблицы)
	...
	ЭлементСправочника.ВидКонтрагента = Перечисления.ЮридическоеФизическоеЛицо.ЮридическоеЛицо;
	...
	КонтактнаяИнформация.Вид = УправлениеКонтактнойИнформацией.ВидКонтактнойИнформацииПоИмени("EmailКонтрагента");
	...
	КонтактнаяИнформация.Вид = УправлениеКонтактнойИнформацией.ВидКонтактнойИнформацииПоИмени("АдресКонтрагента");
