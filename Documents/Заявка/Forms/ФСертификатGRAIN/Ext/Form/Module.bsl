...
Диалог.Фильтр = "Документ Word (*.doc)|*.doc|Word 2007 (*.docx)|*.docx";
...
Исключение
	...
	ПоказатьПредупреждение(Неопределено, "При открытия Word Application произошла ошибка. Проверти правильность установки",10,"Ошибка загрузки шаблона");
...
Транспорт="";
Счетфактура=УС(таб1.Cell(8,2).Range.Text) ;


тнвед.Очистить();

для  ии=10   по 20 цикл
	
знч1=УС(таб1.Cell(ии,2).Range.Text) ;

если не ПустаяСтрока(знч1) тогда
стр=тнвед.Добавить();

стр.номерстрок=  сокрлп(стрзаменить(стрзаменить(таб1.Cell(ии,1).Range.Text,Символ(7),""),".",""));
стр.мест= сокрлп(стрзаменить(таб1.Cell(ии,2).Range.Text,Символ(7),""))    ;
стр.товар=сокрлп(стрзаменить(таб1.Cell(ии,3).Range.Text,Символ(7),""))    ;
//стр.КритерийПроисхождения=сокрлп(стрзаменить(таб1.Cell(ии,4).Range.Text,Символ(7),""))    ;
стр.вес=сокрлп(стрзаменить(таб1.Cell(ии,4).Range.Text,Символ(7),""))    ;
//если ии=10 тогда
//Счетфактура=сокрлп(таб1.Cell(ии,6).Range.Text) ;
//КонецЕсли;
...
странас=сокрлп(стрзаменить(таб1.Cell(ии+3,3).Range.Text,Символ(7),""))   ;

//ПодписантКонтрагента=сокрлп(стрзаменить(таб1.Cell(ии+3,2).Range.Text,Символ(7),""))    ;

ДатаДекларации=сокрлп(стрзаменить(таб1.Cell(ии+7,3).Range.Text,Символ(7),""))  +" 12:00:00"   ;
