...
Процедура ЗагрузитьизДокумента(имяфайла)
	...
	КолСтрокВТ2=таб2.Rows.Count;
	для  ии=2   по КолСтрокВТ2 цикл
		
		//знч2=УС(таб2.Cell(ии,2).Range.Text);
		знч1=УС(таб2.Cell(ии,1).Range.Text);
		знч3=УС(таб2.Cell(ии,3).Range.Text);
		//сообщить(знч3);
		
		если не ПустаяСтрока(знч1) и не ПустаяСтрока(знч3) тогда
			стр=тнвед.Добавить();
			
			стр.номерстрок=  стрзаменить(УС(таб2.Cell(ии,1).Range.Text),".","");
			стр.мест= УС(таб2.Cell(ии,2).Range.Text)    ;
			стр.товар=УС(таб2.Cell(ии,3).Range.Text)    ;
			стр.КритерийПроисхождения=УС(таб2.Cell(ии,4).Range.Text)    ;
			стр.вес=УС(таб2.Cell(ии,5).Range.Text)    ;
			стр.ид=ид;
			если ии=2 тогда
				Счетфактура=УС(таб2.Cell(ии,6).Range.Text) ;
			КонецЕсли;
	...
	странас=УС(таб3.Cell(2,2).Range.Text)   ;
	
	ПодписантКонтрагента=УС(таб3.Cell(3,2).Range.Text)   ;
	
	ДатаДекларации=УС(таб3.Cell(4,2).Range.Text)+" 12:00:00"   ;
...
//&НаКлиенте
//Процедура ТНВЕДПослеУдаления(Элемент)
//	Элементы.ТНВЕД.Обновить();
